...
import { cloneDeep, unset, isNaN } from 'lodash'
import moment from 'moment'

const parserString = (value: any) => {
  return typeof value === 'string' ? JSON.parse(value) : value || []
}
const containTanggal: any[] = []
...
function getFilteredQuery(model?: ModelCtor<any>, prefixName?: string) {
  ...
  sequelizeQuery.addQueryBuilder(
    (filterData: { id: string; value: any }, queryHelper) => {
      ...
      } else if (moment(value).isValid() && isNaN(Number(value))) {
        queryHelper.setQuery(curId, {
          [Op.substring]: moment(value).format('YYYY-MM-DD'),
        })
      } else {
        queryHelper.setQuery(curId, {
          [Op.substring]: `%${value}%`,
      ...
      if (curId === 'tanggalReservasi') {
        queryHelper.setQuery(curId, {
          [Op.substring]: moment(value).format('YYYY-MM-DD'),
        })
        containTanggal.push(value)
      }
      if (curId === 'createdAt') {
        const dayOfMonth = moment().daysInMonth()
        const range = dayOfMonth - Number(value.split('-').pop())
        const startDate = Number(moment(value).format('DD'))
        queryHelper.setQuery(curId, {
          [Op.between]: [
            moment(value)
              ...
              .add(1, 'days')
              .format('YYYY-MM-DD'),
            moment(value).add(range, 'days').format('YYYY-MM-DD'),
          ],
        })
      }
      if (curId === 'id') {
...
function generate(
  ...
  let modFilter: any = filter
  if (filter.tanggalAwalCheckIn && filter.tanggalAkhirCheckOut) {
    modFilter = {
      [Op.or]: [
        {
          tanggalAwalCheckIn: filter.tanggalAwalCheckIn,
        },
        {
          tanggalAkhirCheckOut: filter.tanggalAkhirCheckOut,
        },
      ],
    }
  }
